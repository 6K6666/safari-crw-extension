...
`npm run build` is an alias for:

```shell
npm run build-safari
```

The Safari extension is packaged through the generated Xcode project. After changing extension source, sync the Safari build into the Xcode resources:

```shell
npm run sync-safari-resources
```

Confirm the synced resources match the Safari build:

```shell
diff -qr "dist/safari" "safari/Consumer Rights Wiki/Shared (Extension)/Resources"
```

See `docs/SAFARI.md` for macOS, iOS, and iPadOS testing steps.
