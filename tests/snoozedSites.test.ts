...
test("normalizeSnoozedSiteMap normalizes domain keys", () => {
  const now = Date.now();
  const input = {
    "WWW.Example.COM": [{ incidentSignature: "inc-a|active", snoozedAt: now }],
    "  shop.test.com  ": [
      { incidentSignature: "inc-b|active", snoozedAt: now },
    ],
  };
...
test("normalizeSnoozedSiteMap merges duplicate normalized domains", () => {
  const now = Date.now();
  const input = {
    "www.example.com": [{ incidentSignature: "sig-a", snoozedAt: now }],
    "Example.com": [{ incidentSignature: "sig-b", snoozedAt: now }],
  };
