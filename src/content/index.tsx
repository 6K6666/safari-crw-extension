...
const DEFAULT_INLINE_POPUP_Z_INDEX = "2147483647";
const EMPTY_INLINE_POPUP_MIN_HEIGHT = "152px";
...
const openOptions = () => {
  void (async () => {
    try {
      await openExtensionOptions("content");
    } catch (error) {
...
const ensurePopupRoot = (): Root => {
  ...
  popupShadowMount.style.display = "block";
  popupShadowMount.style.width = "100%";
...
const applyPopupHostPlacement = (position: PopupPosition): void => {
  if (!popupHost) return;

  const placement = getCurrentPopupPlacementStyle(position);
  popupHost.style.cssText = "";
  popupHost.style.all = "initial";
  popupHost.style.display = "block";
  popupHost.style.position =
    typeof placement.position === "string" ? placement.position : "fixed";
  popupHost.style.width =
    typeof placement.width === "string" ? placement.width : "460px";
  popupHost.style.maxWidth =
    typeof placement.maxWidth === "string"
      ? placement.maxWidth
      : "calc(100vw - 32px)";
  popupHost.style.zIndex =
    typeof placement.zIndex === "number"
      ? String(placement.zIndex)
      : DEFAULT_INLINE_POPUP_Z_INDEX;
  popupHost.style.top = typeof placement.top === "string" ? placement.top : "";
  popupHost.style.right =
    typeof placement.right === "string" ? placement.right : "";
  popupHost.style.bottom =
    typeof placement.bottom === "string" ? placement.bottom : "";
  popupHost.style.left =
    typeof placement.left === "string" ? placement.left : "";
  popupHost.style.transform =
    typeof placement.transform === "string" ? placement.transform : "";
};
...
const renderInlinePopup = async (
  ...
  applyPopupHostPlacement(popupPosition);
  if (visibleMatches.length === 0) {
    if (popupHost) {
      popupHost.style.minHeight = EMPTY_INLINE_POPUP_MIN_HEIGHT;
    }
    if (popupShadowMount) {
      popupShadowMount.style.minHeight = EMPTY_INLINE_POPUP_MIN_HEIGHT;
    }
    flushSync(() => {
      root.render(
        <InlineEmptyState
  ...
  if (popupHost) {
    popupHost.style.minHeight = "";
  }
  if (popupShadowMount) {
    popupShadowMount.style.minHeight = "";
  }
...
const runContentScript = async () => {
  ...
  try {
    ...
    const instruction = getInlinePopupInstruction(response);
    if (instruction) {
      await handleInlinePopupInstruction(instruction);
    }
  } catch (error) {
    console.error(
      `${Constants.LOG_PREFIX} Failed to update page context`,
      error,
    );
  }
};
...
const handleInlinePopupInstruction = async (
  ...
  if (instruction.toggle) {
    ...
    await renderInlinePopup(instruction.matches, true);
    return;
  }

  if (!instruction.ignorePreferences) {
    if (forcePopupVisible) {
      if (instruction.matches.length === 0) return;
      if (!(await isWarningsEnabled())) return;
    }
    await renderInlinePopup(instruction.matches, false);
    return;
  }
...
browser.runtime.onMessage.addListener((msg: unknown) => {
  ...
  return handleInlinePopupInstruction(instruction).then(() => true);
});
