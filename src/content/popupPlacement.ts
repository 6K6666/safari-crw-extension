...
const SAFE_AREA_FALLBACK_PX = "0px";

const safeAreaOffset = (side: "top" | "right" | "bottom" | "left"): string => {
  return `calc(${POPUP_EDGE_OFFSET_PX}px + env(safe-area-inset-${side}, ${SAFE_AREA_FALLBACK_PX}))`;
};
...
export const getInlinePopupPlacementStyle = (
  ...
  return {
    ...
    ...(isBottom
      ? { bottom: safeAreaOffset("bottom") }
      : { top: safeAreaOffset("top") }),
    ...(isLeft
      ? { left: safeAreaOffset("left") }
      : { right: safeAreaOffset("right") }),
  };
};
